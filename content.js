...
import {
    extension_settings,
    getContext, // MOVED TO THE CORRECT IMPORT STATEMENT
} from '../../../extensions.js';
    ...
    for (const sentence of sentences) {
        for (let n = NGRAM_MIN; n <= NGRAM_MAX; n++) {
            ...
            for (const ngram of ngrams) {
                ...
                if (normalizedNgram.length > 15) {
                    ngramFrequencies[normalizedNgram] = (ngramFrequencies[normalizedNgram] || 0) + 1;
                }
            }
        }
    }
...
async function generateAndSaveDynamicRules() {
    if (isProcessing) return 0;
    isProcessing = true;
    let addedCount = 0;
    console.log(`${LOG_PREFIX} Starting dynamic rule generation...`);

    const SLOP_THRESHOLD = 3;
    const BATCH_SIZE = 5;

    const allCandidates = Object.entries(ngramFrequencies)
        .filter(([_, count]) => count >= SLOP_THRESHOLD);

    allCandidates.sort((a, b) => b[1] - a[1]);

    const topCandidates = allCandidates.slice(0, BATCH_SIZE).map(([phrase, _]) => phrase);

    if (topCandidates.length === 0) {
        console.log(`${LOG_PREFIX} No new slop candidates found.`);
        isProcessing = false;
        return 0;
    }
    ...
    const userPrompt = `Top ${topCandidates.length} detected repetitive phrases:\n- ${topCandidates.join('\n- ')}\n\nGenerate the JSON array of regex rules for these phrases:`;
...
async function manualAnalyzeChatHistory() {
    ...
    const newRulesCount = await generateAndSaveDynamicRules();

    if (newRulesCount > 0) {
        toastr.success(`Prose Polisher: Analysis complete. AI generated and saved ${newRulesCount} new correction rules!`);
    } else {
        toastr.info("Prose Polisher: Analysis complete. No new rules were needed.");
    }
...
function showFrequencyLeaderboard() {
    const frequencyEntries = Object.entries(ngramFrequencies);
    let contentHtml;
    if (frequencyEntries.length === 0) {
        ...
        contentHtml = `<p>The following phrases have been detected as repetitive in the current session. Phrases with a count of 3 or more will be used to generate new rules.</p><table class="prose-polisher-frequency-table"><thead><tr><th>Repetitive Phrase (n-gram)</th><th>Count</th></tr></thead><tbody>${tableRows}</tbody></table>`;
    }
...
async function initializeProsePolisher() {
    try {
        ...
        clearFreqBtn.addEventListener('click', () => { ngramFrequencies = {}; messageCounter = 0; toastr.success("Prose Polisher: In-memory frequency data has been cleared!"); });
